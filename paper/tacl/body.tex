...
word copied across inscriptions scores zero. (Its within-form-shuffle
false-positive calibration and its saturated-power point are reported in
Appendix~A.2.)
...
population, and the current census count are given in Appendix~C.5.)
...
process. (Its divergence from the calibration targets and the root-template
total-variation-distance mechanics are reported in Appendix~C.5.)
...
but by the direct empirical calibration below, and holds conditional on the
tested null-generating process, not as a universal guarantee. This is a direct
...
bar over the Packard/random-lexeme multiplicity null. These are
...
table. (A self-audit log of confounds this discipline caught inside logos's
own builds is in Appendix~D.)
...
kept distinct (enumerated in Appendix~C.4). On the narrower parsed
syllable-sign budget ($N \approx 5{,}792$;
\citealp[\S1--3]{salgarella2025}) Shannon's unicity distance
...
stable above the bigram floor, robust to the genre-bucketing choice (the H3
nominal near-miss surfacing only in the doubly-relaxed corner is detailed in
Appendix~A.1). A null that names its threshold, its sensitivity, and its
...
the object or its data (\S1--2; see Appendix~D for its relation to the
survey's fifteen challenges and to the evaluation problem).
...
graduation clause; \S3, Appendix~D): the single sign the whole
...
material, and the artifact is fixed before any claim. (The held-out
libation-formula corpus and its low-power caveat are detailed in
Appendix~A.3.)
...
decipherment (the stance of \S4). The
...
the etymological method is ``fraught with problems'' and vocabulary
comparison ``not probative.''
...
probes.} Nothing in the battery is read as a decipherment. (The
sufficiency-sweep exclusion, the no-selective-reporting note, and the
per-probe reporting-status detail are given in Appendix~D.)
...
\paragraph{A.2 \texttt{S\_morph} detector calibration (relocated from
\S3.2).} After the productivity-gate fix (\S3.2 --- an affix must attach to
$\ge$2 distinct residual stems), \texttt{S\_morph}'s measured false-positive
rate on within-form-shuffled corpora was \textbf{3.0\% (6/200; Wilson 95\% CI
[1.4, 6.4]\%)}, consistent with the nominal one-sided $z \ge 2$ rate
($\approx 2.3\%$) within sampling error. We flag this explicitly as an
\textbf{indicative calibration check, not the operative gate}: a 6/200
estimate is too coarse to certify an $\alpha = 0.01$ threshold on its own, and
it is reported only to show the detector is not grossly mis-calibrated at the
nominal level. The graduation gate does not rest on it --- it applies the
order-statistic bar over the far larger permutation and candidate counts the
deflation machinery tracks (\S3.4). Power was \textbf{100\% (50/50)} at a
single strong planted-affix strength --- a saturated point, not a full
sensitivity curve (contrast the phonology power curve, \S7).

\paragraph{A.3 Held-out design: the libation-formula corpus (relocated from
\S9.3).} The libation-formula corpus ($\sim$50 inscribed objects over $>$5
findspots, spanning peak sanctuaries and caves) offers a natural held-out
cross-validation, but a leave-one-site-out split over so few formula sites has
...
\paragraph{C.5 Literature index and \texttt{L\_fake} canary (construction
detail, relocated from \S3.3).} \emph{Index seed and adversarial population.}
...
every disputed entry is indexed to \emph{catch} regurgitation, never as an
...
Asherah!'' is \citet{best1981}, not Gordon; candidates failing independent
corroboration against \citet{rendsburg1996} (\texttt{qa-pa} = kappu, a
Semitic \texttt{ki-ro}) were rejected rather than indexed. Generated from the
seed (invariant 12), the index currently holds \textbf{63 published claims
across 62 indexed entries (55 single signs)} --- a deliberately conservative,
non-exhaustive seed, not yet the full \S C.1 literature census.
\emph{\texttt{L\_fake} calibration.} Its own divergence from the calibration
...
total-variation distance fell from $\sim$0.84 to $\sim$0.33 end-to-end
($\sim$0.23 standalone) on the Ugaritic$\leftrightarrow$Hebrew gold, and any
...
\textbf{What the gate no longer includes (historical).} The
minimum-description-length check ($k \le$ \texttt{U\_floor}, \S4) was, after
the same review, \textbf{removed from the gate and is now reported as a
diagnostic rather than enforced as a graduation clause}: clearing it does not
certify identifiability (it is a necessary-precondition / toy-model
diagnostic, \S4, \S9), and as a hard clause it could only be evaluated against
a free-parameter default that risked passing degenerate inputs.
\texttt{U\_floor} is still displayed next to every claim, and ``parameters
$>$ information $\Rightarrow$ underdetermined'' is still stated (invariant 7)
--- it simply no longer sits inside the AND.

\textbf{Study-wide error budget: elaboration (relocated from \S9.5).} The
sufficiency sweep of the registered sufficiency extension is a known-answer
\emph{upper-bound} diagnostic on benchmarks handed a real cognate signal ---
not a Linear A probe --- so it is not among the five executed probes; on the
corrected matcher its accuracy-vs-size curve is computed and deposited in the
repository (the registered sufficiency extension), and any above-chance
recovery on known-answer benchmarks is true by construction, never a Linear A
positive requiring multiplicity protection. There is no selective reporting:
the one probe that returns an above-chance number --- the cross-script image
leg (\S7.3) --- is reported as a circular \emph{engineering demonstration},
not a positive. Accordingly the segmentation result (\S6) is reported as a
\emph{probe-level} positive, and the cross-script image leg (\S7.3, capped at
$\le 0.75$ and relabeled circular) as a capped demonstration --- neither is
asserted as a battery-level-protected positive.

\textbf{Baseline and self-audit provenance (relocated from \S3.4--3.5).} The
match-rate the deflation is applied to is supplied by a cognate-ID baseline
that reports the \citet{luo2019} accuracy metric; the instance we run is
\citeauthor{tamburini2025}'s CSA\_OptMatcher (coupled simulated annealing,
\S2.3--2.4, reproduced in the artifact), so the gate does not depend on any
single citation. The persisted-artifact discipline caught confounds inside
logos's own builds --- a \texttt{S\_morph} statistic that conflated repetition
with morphology, a verdict clause that was a tautology and could never fire
False, and a search-log tripwire that flagged honest multi-sign logs. Each was
fixed and locked by regression tests before commit, illustrating the harness
turned on itself.

\textbf{The axis, relative to the survey (relocated from \S9.1).} As set out
in the Introduction, the false-positive / multiplicity axis is a property of
inference methodology and epistemics, not of the object or its data; it is
complementary to, not a member of, the fifteen data-and-object obstacles
enumerated by \citet{braovic2024}, and distinct from the evaluation problem
their \S6.3 gestures at.
