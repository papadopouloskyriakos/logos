...
\textbf{generator-conditional negative-control distribution} (\S3.3;
a broader control ensemble spanning unigram, root-template, and
real-language generators is disclosed as future work, not built here);
...
The axis targeted is the \textbf{false-positive / multiplicity axis} ---
a property of inference methodology, not of the object or its data
(\S1--2).
...
assigned to fit the desired root; the refutation rests on the claim's own published table (\S5).
...
claim} --- the deliverable is calibrated absence, not a reading. Every REJECT / NULL bounds \emph{evidential support} on held-out
data, not truth (\S3.4). The one probe that recovers an above-chance
...
\fi
The licensed corpora are not redistributed (Software \& data note);
the harness is reproducible against a locally obtained corpus. This work
is independent and not affiliated with, or endorsed by, any of the cited
projects or their authors.
