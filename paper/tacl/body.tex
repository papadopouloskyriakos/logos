...
problem (what to measure against), separable from the false-positive /
multiplicity problem (whether a fitted reading is being mistaken for a
real one); it is the latter this paper instruments.
...
The artifact is open and archived (\S9.7; Software \& data note,
...
its source, the falsifiable test, and the acceptance threshold, externally
anchored so the model cannot be tuned to them after the fact (the anchoring
mechanics and the per-affix acceptance criteria are given in
Appendix~A.1). Outcomes are enumerated in advance --- CONFIRM, EXTEND,
...
space, and it fails closed on a degenerate null (Appendix~D). The gate emits one
...
are in Appendix~D). The remaining pre-registered clauses ---
\texttt{L\_fake} separation, the corrected margin, and the
order-statistic bar over the multiplicity null --- are conjoined as
specified in Appendix~D. These are
...
kept distinct (Table~\ref{tab:denoms}; Appendix~C.4). On the narrower parsed
...
benchmark, conditional on a known cognate relationship} (its two-way
reading rules are specified in Appendix~C.3; neither branch is ever a
decipherability claim). The accuracy-vs-size curve is deposited in the repository as
...
($U \approx 204$--$415$ signs; \S4) is a \emph{precondition} diagnostic
...
Davis's \texttt{i-*301} verb stem). Pre-registration anchoring (\S3.1):
the pre-registration file's git commit is
\ifanon
...
\fi
serving as the pre-specification timestamp. Each pre-registered affix
...
(Table~\ref{tab:calib}; contrast the phonology power curve, \S7). We flag
...
0.01$ threshold on its own, and the graduation gate does not rest on it
--- it applies the order-statistic bar over the far larger permutation
and candidate counts the deflation machinery tracks (\S3.4).

\paragraph{A.3 Held-out design: the libation-formula corpus (\S9.3).} The libation-formula corpus ($\sim$50 inscribed objects over $>$5
...
signs to detect a weak one'' (\S7.2). The obvious next move --- impute sound
values cross-script from a larger Linear B corpus --- is now \emph{tested
rather than deferred} (\S B.3): more Linear B data is necessary but not
sufficient.
...
harness reframes the withdrawn unicity-distance diagnostic (\S C.4) as a
sharper, answerable question: \emph{is a Linear-A-scale corpus
(Table~\ref{tab:denoms}) large enough to recover a decipherment IF a
known cognate existed?} We report the design and its reading rules here as a known-answer
upper-bound protocol; the full accuracy-vs-size curve, with per-size
permutation floors, completes in the repository (this paper asserts no
interim result from it).
...
reproduction reaches the published Table-3 values). The phonological /
sound path, by contrast, is no longer data-deferred: the D\=AMOS Mycenaean
holdings are now harvested and ingested, and even the full corpus leaves
the cross-script distributional alignment at chance (\S7.3, \S B.3) --- a
property of the signal, not a promissory note against data yet to arrive. We
publish the harness and its bounding logic (Software \& data note,
following Appendix~D).
...
must be labelled where it is used: the working repertoire $V = 259$
conflates syllabograms, subscripts, and logograms. The channel through
...
identifiable; and if Linear A underspells like Linear B, the
sign$\rightarrow$sound channel is lossy and non-injective, and such a
channel may possess no unicity distance at all.
...
\citep[\emph{Order Statistics}, 3rd ed.]{david2003}. The
...
logos working repertoire ($V$) & 259 \\
