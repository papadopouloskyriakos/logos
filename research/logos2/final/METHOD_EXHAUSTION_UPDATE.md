# Method exhaustion update — LOGOS-2 (extends the 72h campaign's exhaustion map)

"Conditionally exhausted" = re-running the method on the SAME corpus with variant settings
is barred by the campaign stop-list; each entry names the exact condition that would
legitimately reopen it. Nothing here claims Linear A is undecipherable — the exhaustion is
of *methods at current evidence*, per the constitution's mission clause.

## Conditionally exhausted by LOGOS-2

| method family | terminal result | reopening condition |
|---|---|---|
...
| Retrieval as an evidential aid (E211) | RAG_USEFUL_FOR_AUDIT_ONLY; NOT_A_CHANNEL | nothing reopens this as an *evidential* channel by construction |

## Standing exhaustions inherited (not re-run, per stop-list)
Hard-pin variants; grade-prior variants; the source-label route (CLOSED NO_POWER) and the
word-context→channel route (REFUTED, well-powered A12) from prior programmes; unrestricted matching/tournaments/OT; A-morphology
re-operationalizations. See the methodology master inventory
(`experiments/linear_a_methodology_inventory`, branch
`audit/linear-a-methodology-master-inventory`) before proposing any "new" method.

## The single exact reopening event
