    ...
    for fname in ("HTtexts.html", "misctexts.html", "religioustexts.html"):
        ...
        for kind, payload in events:
            ...
            all_rows.append(dict(source_file=fname.split(".")[0], doc_id=doc, site=site,
                                 ...
                                 restored=res, damaged=dam,
                                 is_kuro=1 if "KU-RO" in statement else 0, parser="D"))
    with open(os.path.join(HERE, "parser_d_output.csv"), "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=SCHEMA["fields"])
        w.writeheader()
        for r in all_rows:
            w.writerow(r)
    print(f"parser D: {len(all_rows)} records, "
