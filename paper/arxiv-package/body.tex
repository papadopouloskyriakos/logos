...
benchmark, conditional on a known cognate relationship}, readable two
ways: at-floor at Linear-A scale $\Rightarrow$ information-insufficient
...
be implicated. Neither branch is ever a decipherability claim. The
accuracy-vs-size curve is deposited in the repository as a known-answer
upper-bound \textbf{protocol} (Appendix~C.3); this paper asserts no
...
\paragraph{C.3 Information-sufficiency CSA protocol (design; asserts no
result).} The protocol downsamples known-answer benchmarks --- Linear B /
...
rules and the no-result stance are in the body (Registered extension).
