...
1. **The A-initial positional enrichment is real** (E103/E103R) — reduces the space of admissible LA structural
...
5. **The discipline machinery is certified** (E104 pilot → E104R) — the graduation decision rule falsely graduates
   1/400 on signal-free nulls (observed 0.25%; one-sided exact CP95 upper bound 1.18% ≤ the committed 2% bar),
   vs 121/400 with the maxT deflation ablated on the same nulls (suppression factor 121×), while retaining power
   (planted 12% prefix recovered 40/40 — power is established for effects of that general magnitude).
   *Methodological certainty, bound-based.*
...
multi-slot anchor lattice; neither exists in the accessible corpus. A second independent channel is **necessary,
not sufficient** — value assignment additionally requires externally anchored evidence (bilinguals, secure
multi-slot onomastics, metrological ties).
...
- **Detector power** — E104R (bound-based certification) + every epoch's positive control: gates fire, planted
  signals recover.
- **Segmentation uncertainty** — E103/E103R: the graduated finding is robust across the three overlapping
  segmentation/selection variants (3/3).

Ruled IN as *contributing* (but downstream of anchor independence):
- **Source access / palaeographic resolution** — the candidate 2nd channel (stroke contours, whose primary value
  is inventory/transcription) and the external anchor source (Ariadne-2025 Anetaki full edition) are
  `SOURCE_BLOCKED`.
