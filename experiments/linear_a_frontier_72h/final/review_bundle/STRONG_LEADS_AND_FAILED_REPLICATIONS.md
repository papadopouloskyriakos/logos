...
## Failed replications / non-decisive leads (adjudicated verdicts)
...
| **Global network-flow assignment** | E101 | `NO_POWER` (target-free LA) | global optimization needs a known target LA lacks; cannot beat the anchor-lattice null. |
| **Register strata** | E014 | `NO_POWER` | all 3 register pairs at floor after site-discount. |
| **Metrology / fraction values** | E004 (+ prior) | frozen as prospective seal | committed but unopened; see `PROSPECTIVE_SEALS_AND_FUTURE_TESTS.md`. |

## B — COMPLETION_REQUIRED
**None outstanding.** Every epoch carries a terminal verdict + at least the controls its era's schema fielded; the
one detector-power question that could weaken a headline (E016 OOD detector, E099 vs frequency) was itself resolved
in-campaign. E096 is terminal `DE_AUTHORIZED`. No epoch requires a missing control, null, holdout, or mechanical
verdict to be closeable.
