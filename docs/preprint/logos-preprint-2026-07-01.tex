...
whose accuracy-vs-size curve is still completing (§8.3, curve in
progress). Throughout, where a clean number was not yet obtainable we
...
\hypertarget{information-sufficiency-upper-bound-the-csa-known-answer-learning-curve-curve-in-progress}{%
\subsubsection{8.3 Information-sufficiency upper bound: the CSA
known-answer learning curve (curve in
progress)}\label{information-sufficiency-upper-bound-the-csa-known-answer-learning-curve-curve-in-progress}}
...
here as a known-answer upper bound; the full accuracy-vs-size curve is
computing and is released, with per-size chance floors, in the
...
any recovery curve is an \textbf{optimistic benchmark, conditional on a
known cognate relationship} --- a best case Linear A does not enjoy ---
readable two ways: at-floor at Linear-A scale ⇒ information-insufficient
even given a cognate; above-chance ⇒ the binding constraint is
...
Using a data-size recovery curve to bound decipherability is not itself
new --- decipherment learning curves over cipher length (Ravi and Knight
2008; Berg-Kirkpatrick and Klein 2013), the sample-size dependence of
automatic cognate detection (List 2014), and analytic corpus-sufficiency
thresholds (Barber 1974; Zhang and Gong 2016) are the ancestors; our
contribution is the specific repurposing to a cross-language cognate-ID
sufficiency locator for an undeciphered target, not the learning-curve
idea.

The matcher is run on its validated coupled-annealing path, which
reproduces the published known-answer accuracy (at a reduced 2,000-step
budget, Luvian-Hittite at full size recovers ≈ 0.46 against Tamburini's
published 0.475). On that path the non-syllabic known-answer benchmarks
recover well above the analytic chance floor even at small downsample
sizes, whereas the syllabic benchmarks --- Linear-B/Greek and
Cypriot/Greek, the analogues most relevant to syllabic Linear A ---
require a larger corpus before recovery appears, and their
Linear-A-scale points are still computing (per-step cost grows roughly
quadratically in lexicon size). We therefore report §8.3 as a design and
a known-answer upper bound with the curve \textbf{in progress}, and let
the finalized accuracy-vs-size curve stand or fall in the repository,
under the two-way reading above, rather than assert a Linear-A-scale
verdict here. The phonological / sound path, by contrast, is no longer
...
corrected matcher it returns a measured accuracy-vs-size curve (§8.3,
curve in progress), and its above-chance recovery on known-answer
...
\item
  ...
  the logos artifact (v0.1.0), SHA-256
