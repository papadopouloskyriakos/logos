...
challenge}: not a sixteenth property of the data, but the methodological
axis the object-level taxonomy was never meant to cover. It is
named-but-unfilled rather than simply absent, and it is distinct from
the survey's own acknowledged gap --- that on scoring results ``in many
cases there is nothing to compare them against, and their evaluation
remains an open problem'' (Braović et al.~2024, Sec. 6.3, p.~746). That
is an \emph{evaluation-target} problem (what to measure against); the
...
the index stands at \textbf{78 published claims across 61 signs}.
...
virgin sign is present. Two null models must \emph{fail}
(within-inscription sign-order shuffle and the L\_fake corpus) and the
L\_fake corrected margin must be cleared. The minimum-description-length
check (k ≤ U\_floor, §4) was, after the same review, \textbf{removed
from the gate and is now reported as a diagnostic rather than enforced
as a graduation clause}: clearing it does not certify identifiability
(it is a necessary-precondition / toy-model diagnostic, §4, §9), and as
a hard clause it could only be evaluated against a free-parameter
default that risked passing degenerate inputs. U\_floor is still
displayed next to every claim, and ``parameters \textgreater{}
information ⇒ underdetermined'' is still stated (invariant 7) --- it
simply no longer sits inside the AND. These are \emph{gate parameters}
--- pre-registered choices for this corpus, not universal constants.
Validation is leave-one-site-out, not ordinary \emph{k}-fold, because
the libation formula induces formulaic dependence across inscriptions.
The gate is AND-monotone --- clauses can only make it stricter --- so
hardening never lets a previously-failing reading graduate.
...
than adjudicating it mechanically through our own gate.
...
describes i-*301 as morphology ``neither IE nor Afroasiatic'' --- and
Semitic \emph{is} Afroasiatic, so the source ruled out Semitic on
typological grounds a decade earlier. Independently, Salgarella (2025,
§8) concludes Minoan is agglutinative (after Duhoux 1978) and a language
isolate. The claim's own segmentation thus supports the isolate typology
it contradicts by label.
...
chance (0.0135); a from-scratch I-JEPA reaches only 0.322 ± 0.018 and is
excluded from the claims as over-parameterized for \textasciitilde90
glyphs. Image embeddings recover held-out anchors where sequence
co-occurrence sat at chance. Two controls decide how to read that
asymmetry, and together they \emph{downgrade} it from a positive to a
demonstration.
...
control recovers 0.95}, so the harness works and the null is a real
...
contamination rate came out at \texttt{1.000} in all 32 seeds where it
...
\texttt{SU-PA-RA}, alongside the \texttt{KU-RO} item below) are among
Gordon's five West-Semitic equations; \texttt{A-SA-SA-RA-ME} is Best's,
not Gordon's. An external red-team correction split \texttt{KU-RO}: of
its 14/40 hits, only 8/40 route through the Semitic root \emph{kull},
with 7/40 reachable via the general-knowledge meaning ``total''
(confirmed by tablet arithmetic), so \texttt{KU-RO} = ``total'' is
...
Linear-A-scale corpus (\textasciitilde650 distinct word-forms; V = 259
sign-types at N ≈ 5,792 parsed syllable-signs) large enough to recover a
decipherment IF a known cognate existed?} We report the design and its
reading rules here, not an outcome: the sweep is not yet run.
...
One thing is deferred, deliberately: the full CSA sweep is CUDA-grade
and \textbf{deferred to the H100}; the locator currently returns
\texttt{PENDING\_CSA\_SWEEP}, and these cores are not part of this
release. The phonological / sound path, by contrast, is no longer
...
cross-script distributional alignment at chance --- so the bound is a
...
10.5281/zenodo.21087572) so the CSA result, when the H100 cores run, can
be reproduced rather than asserted.
...
pre-registration}: the six pre-registered probes reported here
(morphology, metrology, phonology, cross-script image alignment,
contamination, sufficiency) each carry their own pre-committed α, and
--- critically --- every probe is reported regardless of outcome. There
is no selective reporting across the battery: every probe is reported
regardless of outcome, and the one probe that returns an above-chance
number --- the cross-script image leg (§7.3) --- is reported as a
circular \emph{engineering demonstration}, not a positive. What we have
\emph{not} yet done is deflate across the battery itself: a formal
family-wise or false-discovery correction over the six probe-level tests
is flagged here as future work rather than claimed as already paid. The
direction of that omission is bounded and entirely conservative: all six
probes are reported as nulls or as a capped-and-circular demonstration,
so a battery-level correction can only make each \emph{more} null.
Nothing in the battery is read as a decipherment.
