"""Tests for the §C.2 L_not_indexed generalization test (scripts/comparison/l_not_indexed).
...
run on the real corpus + small deterministic fixtures. Properties locked in:
  ...
  (d) analyze()'s excess-over-random is ~0 for a uniformly-drawn sign and >0 for a concentrated one
      (the sample-size control), the known−not_indexed gap + permutation p, and the matched-n path;
  (e) doctrine grep — l_not_indexed.py does NOT import scripts.verdict (the proposer never grades).
...
from scripts.comparison import l_not_indexed  # noqa: E402
...
def test_partition_by_class_synthetic_excludes_nonphonetic():
    ...
    part = l_not_indexed.partition_by_class(ont)
    assert part["L_known"] == {"DA", "KU"}
    # *301 carries Di Mino's published (quarantined) litindex proposal -> NOT not_indexed (could be memorized);
    # only the genuinely-unproposed *118 remains in L_not_indexed.
    ...
    assert not (part["L_known"] & part["L_not_indexed"])


def test_partition_by_class_on_real_ontology():
    part = l_not_indexed.partition_by_class(l_not_indexed.load_ontology())
    # AB syllabograms (memorizable) land in L_known ...
    assert {"DA", "KU", "SI", "RO"} <= part["L_known"]
    # ... the genuinely-unproposed *-series land in L_not_indexed ...
    assert "*118" in part["L_not_indexed"]
    # ... but *301 (Di Mino's published reading) is QUARANTINED out of not_indexed (could be memorized) ...
    assert "*301" not in part["L_not_indexed"] and "*301" in part["lit_quarantined"]
    # ... and nothing non-phonetic leaks into either phonetic class.
    phonetic = part["L_known"] | part["L_not_indexed"]
...
def test_target_probe_forms_contain_target_and_are_real_context():
    ont = l_not_indexed.load_ontology()
    insc = l_not_indexed.load_inscriptions()
    pairs = l_not_indexed.target_probe_forms(ont, insc, n_known=3, n_not_indexed=3, n_context=5, seed=0)
    assert len(pairs) == 6                                  # 3 known + 3 not_indexed, all covered
...
def test_target_probe_forms_window_is_fixed_across_calls():
    ...
    ont = l_not_indexed.load_ontology()
    insc = l_not_indexed.load_inscriptions()
    a = l_not_indexed.target_probe_forms(ont, insc, n_known=4, n_not_indexed=4, n_context=6, seed=0)
    b = l_not_indexed.target_probe_forms(ont, insc, n_known=4, n_not_indexed=4, n_context=6, seed=0)
...
def test_coverage_report_flags_signs_without_context():
    ont = l_not_indexed.load_ontology()
    insc = l_not_indexed.load_inscriptions()
    cov = l_not_indexed.coverage_report(ont, insc, n_context=6, seed=0)
    ...
    assert cov["L_not_indexed"]["n_covered"] >= 1
    assert cov["L_not_indexed"]["n_covered"] + cov["L_not_indexed"]["n_skipped_no_context"] == \
        cov["L_not_indexed"]["n_candidates"]


# --------------------------------------------------------------------------- #
# (c) proposer loop — accumulation + fail-closed dead call (monkeypatched generate)
# --------------------------------------------------------------------------- #
def test_run_l_not_indexed_accumulates_per_sign_values(monkeypatch, tmp_path):
    canned = (
        "```json\n"
    ...
    monkeypatch.setattr(l_not_indexed.ablation.ollama_client, "generate", fake_generate)

    probe_pairs = [
        ("KU", l_not_indexed.ablation.build_form("F1", ["KU", "RO"])),
        ("*301", l_not_indexed.ablation.build_form("F2", ["*301", "DA"])),
    ]
    run = l_not_indexed.run_l_not_indexed("m", n_seeds=3, probe_pairs=probe_pairs,
...
def test_run_l_not_indexed_dead_call_contributes_nothing(monkeypatch, tmp_path):
    def dead_generate(model, prompt, *, options=None, timeout=180, host=None):
        return {"response": "", "ok": False, "prompt_tokens": 0, "eval_tokens": 0,
                "eval_seconds": 0.0}

    monkeypatch.setattr(l_not_indexed.ablation.ollama_client, "generate", dead_generate)
    probe_pairs = [("KU", l_not_indexed.ablation.build_form("F1", ["KU", "RO"])),
                   ("*301", l_not_indexed.ablation.build_form("F2", ["*301", "DA"]))]
    run = l_not_indexed.run_l_not_indexed("m", n_seeds=4, probe_pairs=probe_pairs,
                              log_path=str(tmp_path / "log.jsonl"))
...
def test_run_l_not_indexed_isolates_a_raising_generate(monkeypatch, tmp_path):
    def boom(model, prompt, *, options=None, timeout=180, host=None):
        raise RuntimeError("host down")

    monkeypatch.setattr(l_not_indexed.ablation.ollama_client, "generate", boom)
    probe_pairs = [("KU", l_not_indexed.ablation.build_form("F1", ["KU", "RO"]))]
    run = l_not_indexed.run_l_not_indexed("m", n_seeds=2, probe_pairs=probe_pairs,
                              log_path=str(tmp_path / "log.jsonl"))
...
def test_analyze_excess_zero_for_uniform_positive_for_concentrated():
    ...
    partition = {"L_known": {"KCONC"}, "L_not_indexed": {"VUNIF"}}

    res = l_not_indexed.analyze(per, partition, seed=0, n_min=10, n_trials=4000, n_perm=500)
...
def test_analyze_n1_sign_is_not_mechanically_inflated():
    ...
    per["SOLO"] = {"v0": 1}
    partition = {"L_known": {"SOLO"}, "L_not_indexed": set()}
    res = l_not_indexed.analyze(per, partition, seed=0, n_trials=2000, n_perm=200)
...
def test_analyze_headline_gap_permutation_and_matched_n():
    ...
    partition = {"L_known": set(), "L_not_indexed": set()}
    # 5 known concentrated (high excess) vs 5 not_indexed uniformly-drawn (excess ≈ 0)
    for j in range(5):
        per[f"K{j}"] = {"v0": 30}
        partition["L_known"].add(f"K{j}")
        per[f"V{j}"] = dict(Counter(rng.choice(values, size=30)))
        partition["L_not_indexed"].add(f"V{j}")

    res = l_not_indexed.analyze(per, partition, seed=1, n_min=10, n_trials=2000, n_perm=2000)
    hl = res["headline"]
    assert hl["excess_gap"] > 0.3                       # known >> not_indexed
    ...
    mn = res["matched_n"]
    assert mn["n_known"] == 5 and mn["n_not_indexed"] == 5
...
def test_analyze_zero_gap_when_classes_identical():
    ...
    for j in range(4):
        ...
        per[f"V{j}"] = dict(base)
        partition["L_not_indexed"].add(f"V{j}")
    res = l_not_indexed.analyze(per, partition, seed=0, n_perm=500)
...
def test_analyze_no_power_when_a_class_empty():
    per = {"KCONC": {"v0": 5, "v1": 5}}
    partition = {"L_known": {"KCONC"}, "L_not_indexed": set()}
    res = l_not_indexed.analyze(per, partition, seed=0, n_perm=100)
...
# --------------------------------------------------------------------------- #
# (e) doctrine grep — l_not_indexed.py never imports scripts.verdict (proposer != grader)
# --------------------------------------------------------------------------- #
def test_l_not_indexed_does_not_import_verdict():
    src_path = os.path.join(_REPO_ROOT, "scripts", "comparison", "l_not_indexed.py")
    ...
    assert not any("verdict" in m for m in imported), (
        f"l_not_indexed.py must not import the verdict machinery; found {sorted(imported)}"
    )
