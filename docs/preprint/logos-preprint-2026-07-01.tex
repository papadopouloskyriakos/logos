...
root matches --- recurs, unpaid, whenever multiplicity goes unaccounted.
The field's flagship survey enumerates fifteen challenges, all
properties of the object and its data; none concern overfitting,
multiple testing, false-positive control, or contamination. We frame
that methodological axis, modestly, as a candidate sixteenth challenge
and build machinery for it: a falsification-first, decontaminated
inference framework that treats every positive as guilty until proven
innocent. Four defences are enforced --- a fabricated-language canary
supplying a false-positive floor, multiple-comparison deflation over
signs, roots, and families tried, a literature index separating
published from literature-unseen signs, and pre-registration with a
held-out gate graded mechanically rather than by the proposing model.
This is a methods paper; we assert no decipherment. Linear A is worked
as the honest null: the binding constraint is identifiability, not
corpus size or effort. We report a body of pre-registered nulls with
their borderline cases named, and analyse the June-2026 *301 ``cracked''
claim as a worked failure case. The artifact is open and archived
...
multiplicity of candidate readings is unpaid, and the sign inventory is
mixed and partly lossy. A lossy, non-injective sign-to-sound channel
need not admit a decipherment at any corpus size, so we make no claim
that ``there is enough text to pin a reading''; the rigorous
demonstration of \emph{that} constraint is the result the field can use.
The June-2026 AI-assisted ``Linear A cracked'' claim --- reading the
\texttt{\textbackslash{}*301}-anchored libation verb as an extinct
Semitic form --- serves throughout as a worked example of exactly these
failure modes, analysed from its public, archived presentation as a
...
\hypertarget{the-survey-spine-and-its-named-but-unfilled-cell}{%
\subsubsection{2.1 The survey spine and its named-but-unfilled
cell}\label{the-survey-spine-and-its-named-but-unfilled-cell}}
...
separated from a false positive. Two field surveys, the same
named-but-unfilled axis. We do not score this against either as an
...
Power was \textbf{100\% (50/50)} at a single strong planted-affix
...
\textbf{(c) Strongest: the morphology contradicts the assigned family.}
The claim's own table draws the verb as concatenative and
prefix-stacking --- {[}prefix A ``I''{]} {[}stem-morpheme TA{]}
{[}stem-vowel I{]} {[}root *301{]} --- i.e.~an \textbf{agglutinative}
form. Semitic morphology is non-concatenative (root-and-pattern); a
prefix-stacked agglutinative verb is its opposite, so the claim is
internally incoherent \emph{from its own figure}, with no
give-versus-dwell adjudication required. This is corroborated from both
directions in the primary literature. Davis (2013, p.~38 n.~13)
characterises i-*301's morphology as ``neither IE nor Afroasiatic'' ---
and Semitic \emph{is} Afroasiatic, so a decade earlier the primary
description already found \textbf{no Afroasiatic (hence no Semitic)
...
1978) and a language isolate. The claim's own segmentation thus supports
the isolate typology it contradicts by label.
...
assignment, unpaid multiplicity, and a sign whose value is chosen rather
than anchored.
...
Godart and Olivier 1985; Timm 2008) --- i.e.~over distinct word
\emph{types}, not tokens. Restricting our corpus to distinct multi-sign
types likewise recovers 3.07; the token vs.~type denominator is the
whole discrepancy. We note one tension worth flagging rather than
burying: Fuls reads the 3.3-sign length, via his exponent-to-word-length
regression, as evidence that Minoan is \emph{polysynthetic} (rich
affixation); our direct morphology test finds the apparent affixation
bigram-reproducible with no power, so it does \textbf{not} corroborate a
rich-morphology reading --- the word-length typology and the direct null
point opposite ways, and we report the null. This bigram free-rider is
one of the would-be confounds the machinery has caught and reported as a
null rather than a finding. One validation this null still lacks --- and
the first item of the planned revision --- is a \textbf{positive
control}: metrology and phonology (§7.1--7.2) each demonstrate their
detector fires on a planted signal, but the morphology bigram-floor test
has not yet been shown to recover \emph{known} affixation, so ``no power
on Linear A'' cannot yet be cleanly separated from ``the floor is too
strong.'' The direct control is the identical bigram-floor test on
\textbf{Mycenaean Greek (Linear B)}, whose longer words carry
uncontested inflectional morphology; the DĀMOS Linear-B corpus ingested
for §7.3 (13,562 wordforms) now makes it runnable. Our short-word
argument makes a falsifiable prediction there --- the test \emph{should}
regain power on Linear B --- which, if borne out, would establish the
Linear A result as a short-word / low-power situation rather than a
claim that Minoan lacks morphology; until that control is run we report
the Linear A outcome strictly as \emph{no power}, not as evidence
against affixation.
...
information-sufficiency sweep is presented as \textbf{protocol and
bounding logic, not a result} --- its compute cores are deferred (§8.3).
Throughout, where a clean number was not yet obtainable we report the
...
\hypertarget{information-sufficiency-upper-bound-protocol-and-bounding-logic-sweep-pending}{%
\subsubsection{8.3 Information-sufficiency upper bound: protocol and
bounding logic (sweep
pending)}\label{information-sufficiency-upper-bound-protocol-and-bounding-logic-sweep-pending}}
...
a known cognate existed?} We report the design, its reading rules, a
validated implementation, and a reduced-budget end-to-end run here ---
but not yet the \emph{converged} curve: the matcher is built and
exercised, and the sweep is compute-bound (see below), so we provide the
runnable harness and the measured lower bounds rather than assert a
converged outcome.
...
identifiability --- cognate-absence and unpaid multiplicity over a mixed
sign inventory --- not corpus size. Neither branch is ever a
decipherability claim, and neither asserts ``there is enough text.''

One thing is deferred, deliberately: the full CSA learning-curve sweep.
The cognate-matcher itself is implemented and validated --- a
batched-parallel CUDA energy that evaluates all sixteen coupled
annealers in one edit-distance kernel per step, verified
\textbf{bit-identical} to the serial reference on CPU (matching final
energies and states) and memory-stable under long runs. What remains
compute-bound is the sweep \emph{breadth}: hundreds of anneals
(benchmarks × corpus sizes × seeds), each thousands of steps of an
orchestration-heavy search. We therefore ship the runnable harness
(driver, batched energy, and its validation) and the per-size chance
floor and Linear-A locator, so the accuracy-vs-size curve can be
\textbf{reproduced and audited rather than asserted}. We did run it
end-to-end at a \emph{reduced} budget (300--500 steps, single seed;
\texttt{results/csa/curve\_lowerbounds.json}): all six measured points
--- luvian-Hittite at sizes 20--59 and Linear B/Greek at sizes 100--200
--- sit at or within noise of the analytic chance floor. But those are
\textbf{under-converged lower bounds} (the CSA reproduction reaches its
published accuracy only near \textasciitilde4,000 steps), and the
per-step cost grows roughly quadratically in lexicon size (linb size-100
≈ 4 min, size-200 ≈ 17 min), so a \emph{converged} sweep across the
primary benchmark's full size range is hours-to-days of compute. The
honest state is therefore \textbf{implemented, validated, and exercised
--- but not converged}: at this budget the curve is uninformative about
sufficiency (it cannot separate under-convergence from
information-insufficiency), and the converged curve is the deferred
future work. The phonological / sound path, by contrast, is no longer
data-deferred: the DĀMOS Mycenaean holdings are now harvested and
ingested, and §7.3 reports that even the full corpus leaves the
cross-script distributional alignment at chance --- so that bound is a
property of the signal, not a promissory note against data yet to
arrive. We publish the harness and its bounding logic (concept DOI
10.5281/zenodo.21087572).
...
gestures at. We do not claim that axis is simply absent from the prior
taxonomy; we claim it is named-but-unfilled, and logos supplies
mechanical machinery for it.
...
desired Y; the label wins. In the June-2026 *301 claim the claim's own
published table draws a concatenative, prefix-stacking --- hence
agglutinative --- verb, yet the assigned language family is Semitic,
whose morphology is non-concatenative (root-and-pattern). The desired
output (``prayers to a Goddess in a matriarchal society'') operates as a
fixed prior, so the conclusion conforms to the prior rather than
emerging from constrained readings. This is distinct from statistical
overfitting and must be named separately. Notably, the current synthesis
independently concludes Minoan is agglutinative and a language isolate
(Salgarella 2025 §8), so the segmentation corroborates the isolate
typology while contradicting the Semitic label.
...
to, an unpaid multiple-testing search burden, and a mixed sign
...
regardless of outcome. (A sixth, the sufficiency sweep of §8.3, is
specified and its harness validated but was \textbf{not executed to
convergence}, so it is reported as protocol-only, not as a result.)
There is no selective reporting: the one probe that returns an
above-chance number --- the cross-script image leg (§7.3) --- is
reported as a circular \emph{engineering demonstration}, not a positive.
On the battery-level correction the paper's own rule demands: four of
the five executed probes are nulls sitting far from any threshold
(metrology p = 1.0; phonology p = 0.40 / 0.86; morphology at no power;
contamination an artifact by construction), so a family-wise/FDR
correction over the probe-level tests is arithmetically trivial and only
makes each \emph{more} null --- and the fifth probe surfaces \textbf{no
positive for such a correction to protect} (the image leg is capped at ≤
0.75 and relabeled circular). Battery-level multiplicity is therefore
not load-bearing here: there is no surviving positive that a correction
...
in \texttt{docs/related/}. A small number of entries (marked †) await a
final page/venue verification.}
...
\item
  Corazza, M., et al.~(2022). \emph{Sign2Vec}: unsupervised
  deep-learning clustering of the Cypro-Minoan sign repertoire. †
  \emph{Venue pending verification.}
\item
  Daggumati, S., \& Revesz, P. Z. (2019). Data mining ancient scripts to
  investigate their relationships (DEXA). † And (2023),
  \emph{Information} 14(4):227.
\item
...
\item
  ...
  West-Semitic N-W-Y (the June-2026 announcement analysed in §5). †
  \emph{Widely circulated online; venue/date pending verification. The
  reference copy of the claim and its table is the archived record
  carried in the logos artifact (v0.1.0).}
\item
  Duhoux, Y. (1978). On the (agglutinative) typology of the Minoan
  language (as characterised via Salgarella 2025 §8). †
\item
  Duhoux, Y. (1992). Gloss of the Linear A libation-formula verb i-*301
  as ``give/dedicate'' (as adopted by Davis 2013). †
\item
  Duhoux, Y. (1997). The Linear A locative/allative prefix i-/j-
  ``to/at'' (as cited in Salgarella 2025 §8). †
\item
  Ferrara, S., \& Tamburini, F. (2022). Advanced techniques for the
  decipherment of ancient scripts. \emph{Lingue e Linguaggio}
  2/2022:239--259.
\item
  Ferrara, S., Montecchi, B., \& Valério, M. (n.d.). What Is the
  `Archanes Formula'? Deconstructing and Reconstructing the Earliest
  Attestation of Writing in the Aegean. † \emph{Year/venue pending
  verification.}
\item
...
\item
  ...
  \emph{Venue pending verification; the Type 0--III typology is used
  here via Ferrara and Tamburini (2022).}
\item
...
\item
  Min Eu, G., Xu, R., \& Perono Cacciafoco, F. (2019). Linear A
  comparison against time-compatible lexical lists. † \emph{Venue
  pending verification.}
\item
  Packard, D. W. (1974). \emph{Minoan Linear A.} Berkeley: University of
  California Press. †(page cite for the nine fabricated decipherments
  pending)
\item
...
\item
  Tamburini, F. (2025). Coupled simulated annealing for cognate
  identification in undeciphered scripts (CSA\_OptMatcher), evaluated by
  the Luo et al.~(2019) accuracy metric. † \emph{Venue/DOI pending; the
  method is described in §2.3--2.4 and reproduced in the logos artifact,
  so the baseline is specified independently of this citation.}
\item
  Thomas, R. (2020). Some reflections on morphology in the language of
  the Linear A libation formula. \emph{Kadmos.} †
\item
  Timm, T. (2008). Linear A word-list statistics (as cited in Fuls
  2015:134, 227). † \emph{Full venue pending verification.}
\item
  Valério, M. (2007). Analysis of Linear A suffixation (as cited in
  Salgarella 2025 §8). †
\item
