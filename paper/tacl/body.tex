...
\else
  \hypersetup{pdfauthor={Kyriakos Papadopoulos},
    pdftitle={Falsification-First Decipherment},
...
enumerates fifteen ``major challenges,'' from unknown writing system and
unknown language to small dataset, incomplete vocabulary, allographs, and
unavailable parallel data \citep{braovic2024}. Those fifteen are
...
conditions (the list includes resource and hardware items too). What
...
\citeauthor{packard1974}'s \citeyearpar{packard1974} nine deliberately
fabricated decipherments already exercised exactly this false-positive
logic as a control, and the selective-inference machinery we import
\citep{bailey2014} is mature elsewhere --- our contribution is to
...
acknowledged gap --- that on scoring results ``in many cases there is
nothing to compare them against, and their evaluation remains an open
problem'' \citep[Sec.~6.3, p.~746]{braovic2024}. That is an
\emph{evaluation-target} problem (what to measure against); the
false-positive / multiplicity problem (whether a fitted reading is being
mistaken for a real one) is separable from it, and it is the latter this
paper instruments.
...
few formula sites this cross-validation has low statistical power, and we
report it as such rather than as a decisive test \citep{braovic2024}.
...
\ifanon
The artifact is open and archived as an anonymized artifact (code and
pre-registrations released on acceptance).
\else
The artifact is open and archived
(\url{https://github.com/papadopouloskyriakos/logos}; Zenodo concept DOI
...
rather than absent. logos supplies the machinery to police it:
preregistration, a decontaminated derivation/held-out partition,
multiplicity deflation, and a graduation gate computed mechanically rather
than by the model that proposed the reading.
...
by \texttt{verdict.py} --- though we flag at the outset that so few
formula findspots give this particular test low statistical power
(quantified in Section~8). We locate our components in the survey's own
...
mechanical held-out verdicts, released openly
\ifanon
(as an anonymized artifact; code and pre-registrations released on
acceptance),
\else
(\url{https://github.com/papadopouloskyriakos/logos}; Zenodo concept DOI
10.5281/zenodo.21087572),
\fi
with Linear A worked as the honest null rather than a claimed crack.

\section{Methods: the discipline harness}

The harness is a comparison-layer pipeline that a reader can reimplement.
A candidate reading enters as a \emph{proposal} and is subjected, in fixed
order, to five gates: pre-registration, deflation, decontamination,
held-out graduation, and a cross-cutting rule that every gate is scored
from persisted artifacts rather than from any model's narration of its own
success. Model-derived signals --- LLM theses, learned embeddings,
cognate-matcher assignments --- enter capped at confidence $\le 0.75$,
structurally unable to \emph{decide} a verdict; only
mechanically-verified held-out reads rank higher. The design targets the
false-positive / multiplicity / contamination axis that
\citet{braovic2024}'s fifteen-challenge taxonomy names but leaves
unfilled (\S1--2).
...
its source, the falsifiable test, and the acceptance threshold --- with
the file's git commit ---
\ifanon
externally anchored by the immutable artifact deposit ([artifact DOI
withheld for review])
\else
externally anchored by the immutable Zenodo deposit (concept DOI
10.5281/zenodo.21087572)
\fi
--- serving as the pre-specification timestamp, so the model cannot be
tuned to them after the fact. Each pre-registered affix (drawn from
\citet{thomas2020} and Duhoux/Val\'{e}rio) must recur on $\ge$2 distinct
...
0.01$ corrected. Outcomes are enumerated in advance --- CONFIRM, EXTEND,
...
an Euler-$\gamma$ order statistic (\texttt{expected\_max\_order\_stat}($\mu_0,
\sigma_0, N$)) that gives the bar a normal-approximation to the maximum
over $N$ candidates tried that the best should clear by chance.
Multiplicity is instrumented, not hand-passed: a deduplicating
\texttt{SearchLog} reports the exact distinct-candidate count
\texttt{n\_trials} (the deduplicated count of candidate readings attempted)
a scanner produces (invariant 12). The strong structural statistic,
...
word copied across inscriptions scores zero. After that fix, its measured
false-positive rate on within-form-shuffled corpora was \textbf{3.0\%
(6/200; Wilson 95\% CI [1.4, 6.4]\%)}, consistent with the nominal
one-sided $z \ge 2$ rate ($\approx 2.3\%$) within sampling error. We flag
this explicitly as an \textbf{indicative calibration check, not the
operative gate}: a 6/200 estimate is too coarse to certify an $\alpha =
...
(\S3). Power was \textbf{100\% (50/50)} at a single strong planted-affix
strength --- a saturated point, not a full sensitivity curve (contrast the
phonology power curve, \S7).

\subsection{Decontamination}

A large open-weight model has read Gordon, Best, and Di Mino, so a
proposed correspondence that merely reproduces a \emph{published} value is
...
\texttt{L\_known} (referenced by $\ge$1 published proposal) and
...
return what is in the literature. The seed is deliberately conservative
--- a sign wrongly omitted is wrongly granted \texttt{L\_not\_indexed}
status, the dangerous direction --- and every disputed entry is indexed to
...
Gordon's equations and a-sa-sa-ra-me = ``oh Asherah!'' is
\citet{best1981}, not Gordon; candidates failing independent corroboration
against \citet{rendsburg1996} (\texttt{qa-pa} = kappu, a Semitic
\texttt{ki-ro}) were rejected rather than indexed. Generated from the seed
(invariant 12), the index currently holds \textbf{63 published claims
across 62 indexed entries (55 single signs)} --- a deliberately
conservative, non-exhaustive seed, not yet the full \S C.1 literature
census.

\textbf{The \texttt{L\_fake} canary} is a phonotactically-plausible
invented lexicon, calibrated to a real candidate language's phoneme
frequencies, root-template structure, and lexicon size, but never
published and never in any training corpus. Running the full pipeline
against it yields a \textbf{generator-conditional} false-positive floor:
any signal it produces is definitionally spurious, and memorisation cannot
...
process. Its own divergence from the calibration targets is reported,
never minimised; with a Semitic root-template sampler and an external
Hebrew (ETCBC/bhsa) reject set, the root-template total-variation distance
fell from $\sim$0.84 to $\sim$0.33 end-to-end ($\sim$0.23 standalone) on
the Ugaritic$\leftrightarrow$Hebrew gold, and any residual real-lexicon
collision is measured and published rather than asserted to be zero.
...
\texttt{n\_trials} deflation). Explicitly, for a null of mean $\mu_0$ and
standard deviation $\sigma_0$, the bar is
\[
  \resizebox{0.95\columnwidth}{!}{$\displaystyle
  E[\max] = \mu_0 + \sigma_0\bigl[(1-\gamma)\,\Phi^{-1}(1-\tfrac{1}{n})
  + \gamma\,\Phi^{-1}(1-\tfrac{1}{ne})\bigr]$}
\]
($\gamma$ the Euler--Mascheroni constant, $\Phi^{-1}$ the standard-normal
quantile) --- a \textbf{normal-order-statistic approximation} to the
expected maximum over the attempted candidates, where \textbf{$n =$
\texttt{n\_trials} is the \emph{count of candidates the search tried},
not an estimated number of independent hypotheses}. The approximation is
exact only under its Gaussian/independence assumptions; the error-control
claim is carried by the direct empirical calibration below and holds
conditional on the tested null-generating process, not as a universal
guarantee. This is a direct correction for selection across the hypothesis
space, and it fails closed on a degenerate null ($\sigma_0 \le 0
\Rightarrow E[\max] = \mu_0 \Rightarrow$ no graduation). The gate emits one
of four mechanical verdicts: \textbf{GRADUATE} (every clause holds and the
...
calibrated non-result, publishable as such), and \textbf{NO POWER} (the
experiment cannot adjudicate). These verdicts concern \emph{evidential
...
\textbf{INCOMPLETE} (fail-closed); else an observed statistic not exceeding
the null mean (indistinguishable from the \texttt{L\_fake} control)
$\rightarrow$ \textbf{NULL\_PUBLISHED}; else $\rightarrow$ \textbf{REJECT}.
In the calibration below, the instrumented best-of-100 nulls partition as
3 GRADUATE / 10 NULL\_PUBLISHED / 487 REJECT (no INCOMPLETE, the search
being instrumented). We calibrate the refusal rule directly: under the
...
outright and 10 published as calibrated nulls
(\texttt{scripts/\allowbreak gate\_null\_calibration.py}); this is an empirical
error-control result conditional on that null-generating process, not a
universal guarantee. The match-rate the deflation is applied to is
supplied by a cognate-ID baseline that reports the \citet{luo2019} accuracy
metric; the instance we run is \citeauthor{tamburini2025}'s
CSA\_OptMatcher (coupled simulated annealing, \S2.3--2.4, reproduced in the
artifact), so the gate does not depend on any single citation. A second
...
signs above a pre-committed threshold and on a minimum number of distinct
such signs --- so reproducing already-published values cannot by itself
graduate a claim, and the clause fails closed when no threshold or no
supported not-indexed sign is present. The candidate must separate from
the \texttt{L\_fake} negative-control distribution, its corrected margin
must be cleared, and the observed statistic must beat the order-statistic
bar over the Packard/random-lexeme multiplicity null (the within-inscription
sign-order shuffle is a null in the morphology module, \S6, not a gate
clause). The minimum-description-length check ($k \le$ \texttt{U\_floor},
\S4) was, after the same review, \textbf{removed from the gate and is now
reported as a diagnostic rather than enforced as a graduation clause}:
clearing it does not certify identifiability (it is a
necessary-precondition / toy-model diagnostic, \S4, \S9), and as a hard
clause it could only be evaluated against a free-parameter default that
risked passing degenerate inputs. \texttt{U\_floor} is still displayed
next to every claim, and ``parameters $>$ information $\Rightarrow$
underdetermined'' is still stated (invariant 7) --- it simply no longer
sits inside the AND. These are \emph{gate parameters} --- pre-registered
choices for this corpus, not universal constants. Validation is
leave-one-site-out, not ordinary \emph{k}-fold, because the libation
formula induces formulaic dependence across inscriptions. The gate is
...
table. This discipline caught confounds inside logos's own builds --- a
\texttt{S\_morph} statistic that conflated repetition with morphology, a
verdict clause that was a tautology and could never fire False, and a
search-log tripwire that flagged honest multi-sign logs. Each was fixed
and locked by regression tests before commit, illustrating the harness
turned on itself.
\ifanon
The artifact is public and archived as an anonymized artifact (code and
pre-registrations released on acceptance).
\else
The artifact is public and archived (repository
\url{https://github.com/papadopouloskyriakos/logos}; Zenodo concept DOI
10.5281/zenodo.21087572).
\fi
...
finds, of which about 1,400 are readable inscriptions, drawn from 56
findplaces spanning ca. 1800--1450 BCE (Middle Minoan II--Late Minoan IB)
but overwhelmingly concentrated in the single LM IB destruction horizon
\citep[\S1--3, \S6]{salgarella2025}. That near-single horizon is why a
...
distinct denominators (97 token-frequent syllabograms; $\sim$150 full
syllabary; 259 the working repertoire; $\sim$350 with ideograms) and two
occurrence channels that must be kept distinct --- the broader $\sim$7,400
sign occurrences and the narrower $N \approx 5{,}792$ parsed
syllable-signs the unicity computation consumes
\citep[\S1--3]{salgarella2025}. On that budget Shannon's unicity distance
gives $U \approx 204$--$415$ signs, everywhere far below $N$; we retain
this only as a toy-model \emph{precondition} diagnostic and explicitly
withdraw it as any sufficiency or refutation claim (the full denominator
bookkeeping and its derivation are in Appendix~C.4). Qualitatively, the
figure cannot settle identifiability either way: unicity distance
presupposes a known plaintext-language oracle we do not have, and a lossy,
non-injective sign$\rightarrow$sound channel --- if Linear A underspells
like Linear B --- may possess no unicity distance at all. Corpus size, in
short, cannot decide the question.
...
that reproduces a published segmentation is shared-source contamination,
...
uncontested inflection: on the 13,562-wordform D\=AMOS corpus (mean 3.23
signs/word) it confirms affixation at rate 0.562 --- above the same bigram
floor ($\sim$0.30) --- with \texttt{has\_morphology\_power} = True, so the
control rules out a universally insensitive detector and supports the
reading that Linear A's shorter forms (mean 1.84) contribute materially to
its lack of power (Appendix~A). One held-out positive stands and survives
...
(P(gap $>$ 0) = 0.998; the paired-gap detail is in Appendix~A). This
...
Direction D poses the accounting tablets as a constraint-optimisation: can
the fraction-sign values be solved so line items balance the preserved
KU-RO totals? Held-out fraction balance does not beat the shuffle null
...
sign. It replaces the ``just use ML to recover the sounds'' objection with
a quantified detectability floor.
...
\textbf{0.75}, not a positive decipherment claim. The study reports no
positive decipherment claim.
...
partial-null and the confound rather than a headline. (A separate
registered extension --- the known-answer sufficiency protocol --- follows
this section; it asserts no result and is not filed under Results.)

\subsection{LLM-ablation: does a large open-weight model regurgitate the
literature?}

Does a large open-weight model regurgitate the literature rather than
decipher it? In the LLM-ablation, \texttt{qwen2.5:72b} reproduces famous
...
reproduces obscure vessel equations (0/40). Verdict: a reproduction
...
the time on the literature-unseen *-series signs ($\sim$1/40) --- an
abstention asymmetry consistent with differential public exposure (the
experiment does not isolate memorisation from correlated item-level
differences). With no ground truth on the not-indexed signs,
\textbf{consistency is not correctness}: it bounds how the model behaves,
never whether it is right.

\section*{Registered extension: a known-answer sufficiency protocol
(asserts no result)}
...
signal Linear A lacks, any recovery curve is an \textbf{optimistic
benchmark, conditional on a known cognate relationship}, readable two ways:
at-floor at Linear-A scale $\Rightarrow$ information-insufficient even
given a cognate; above-chance $\Rightarrow$ identifiability
(cognate-absence, uncorrected multiplicity) rather than corpus size would
be implicated. The accuracy-vs-size curve is deposited in the repository as
a known-answer upper-bound \textbf{protocol}; this paper asserts no interim
result from it.
...
the pre-registered $\alpha = 0.01$, no pre-registered affix is
cross-stratum stable above the bigram floor, robust to the genre-bucketing
choice; the closest candidates are the Duhoux/Val\'{e}rio H3 nominal
affixes (\texttt{i-} ``to/at'', \texttt{-te/-ti} ``from/of''), which
surface only in the doubly-relaxed corner ($\alpha = 0.05$ \emph{and} loose
bucketing) (\artref). A null that names its threshold, its sensitivity, and
...
anything on a small corpus. As set out in the Introduction, this is a
property of inference methodology and epistemics, not of the object or its
data; it is complementary to, not a member of, the fifteen data-and-object
obstacles enumerated by \citet{braovic2024}, and distinct from the
evaluation problem their \S6.3 gestures at.
...
segments an agglutinative, prefix-stacking verb, yet the assigned language
family is Semitic and the desired output (``prayers to a Goddess in a
matriarchal society'') operates as a fixed prior, so the conclusion
conforms to the prior rather than emerging from constrained readings. This
is distinct from statistical overfitting and must be named separately.
Notably, the current synthesis independently concludes Minoan is
agglutinative and a language isolate \citep[\S8]{salgarella2025}, which the
segmentation matches while the Semitic label does not --- the mismatch is
empirical, between the label and the primary descriptions of the form, not
an a-priori typological impossibility.
...
exactly this --- after pre-submission review it is \textbf{reported next to
every claim rather than enforced as a graduation clause} (\S3), because
clearing it does not certify identifiability: the single sign the whole
reading pivots on carries no cross-script anchor (its AB-number and Linear
B columns are empty), so its value was assigned to fit the desired root ---
the textbook free-parameter case the machinery flags as underdetermined.
The refutation rests on the claim's own published table, archived in
\artobj.
...
held-out $\rightarrow$ locked artifact}: a hypothesis is hash-keyed and
timestamped before it is tested, graded mechanically against held-out
material, and the artifact is fixed before any claim. The libation-formula
corpus ($\sim$50 inscribed objects over $>$5 findspots, spanning peak
sanctuaries and caves) offers a natural held-out cross-validation, but a
leave-one-site-out split over so few formula sites has low statistical
power --- and site-level CV is in any case necessary, not sufficient, since
within-site scribal-hand correlation must also be controlled
\citep{salgarellajudson2024}. This formula corpus should not be conflated
with the 99 stone-vessel inscriptions of the ``libation'' genre stratum
(\S6): they are different objects.
...
over-claim from corpus size. On the real corpus (1,341 inscriptions;
$N \approx 5{,}792$ parsed syllable-signs, $V = 259$) the toy-model unicity
distance is $U \approx 204$--$415$ signs --- a \emph{precondition}
diagnostic only. We explicitly withdraw any reading of this figure as a
sufficiency or refutation claim: a lossy, non-injective sign$\rightarrow$sound
channel may have no unicity distance at all, so ``$U \ll N$'' does
\textbf{not} license the statement that there is enough text to pin a
decipherment (this is the stance of \S4, and \S9 now aligns with it). The
...
establish that more data would be irrelevant. (Note that the $\sim$7,400
sign-occurrence figure sometimes quoted --- Schoep 2002;
\citealp[\S1--3]{salgarella2025} --- counts a different channel than the
5,792 parsed syllable-signs used for $U$ here.) \citet[\S8]{salgarella2025}
reaches the field-level version of this by a different route: the
etymological method is ``fraught with problems,'' vocabulary comparison is
``not probative,'' and she names digital/statistical analysis as ``the next
...
both. Within a single probe, matches are deflated for the signs $\times$
roots $\times$ hypotheses searched (the attempted-candidate-count
correction of \S3--4). Across the study, the unit of multiplicity is the
...
is reported regardless of outcome. (The sufficiency sweep of the registered
sufficiency extension is a known-answer \emph{upper-bound} diagnostic on
benchmarks handed a real cognate signal --- not a Linear A probe --- so it
is not among these five; on the corrected matcher its accuracy-vs-size
curve is computed and deposited in the repository (the registered
sufficiency extension), and any above-chance recovery on known-answer
benchmarks is true by construction, never a Linear A positive requiring
multiplicity protection.) There is no selective reporting: the one probe
that returns an above-chance number --- the cross-script image leg (\S7.3)
--- is reported as a circular \emph{engineering demonstration}, not a
positive. \textbf{Error control is defined within each pre-registered
probe; we claim no global family-wise guarantee across probes.}
Accordingly the segmentation result (\S6) is reported as a
\emph{probe-level} positive, and the cross-script image leg (\S7.3, capped
at $\le 0.75$ and relabeled circular) as a capped demonstration --- neither
is asserted as a battery-level-protected positive. Nothing in the battery
is read as a decipherment.

\subsection{Limitations}

The nulls are honestly bounded, not decisive. A dependence-adjusted
trial-count correction of the morphology z-test is deferred; the current
control is a conservative $\ge 2$-distinct-sites presence gate, which would
only strengthen the null (\artref). No held-out \emph{decipherment} has
...
here claims a rejected reading is \emph{false}: with no Linear A ground
truth, every REJECT / NULL in this study bounds \emph{evidential support}
on held-out data, not truth. The one probe that recovers an above-chance
number, cross-script sign-image alignment (direct-NN 0.410 [0.18, 0.64];
Procrustes 0.185 [0.00, 0.36]), is a font-robust but circular
\emph{engineering demonstration} (\S7.3): a font-swap control refutes the
typeface-artifact worry, but the A$\leftrightarrow$B anchor values were
themselves assigned by sign shape, so the recovery re-derives a
shape-defined mapping and is truth-capped at $\le 0.75$, never a reading. No
phonetic claim is made. The philological points in \S5 rest on the
published literature \citep{davis2013, duhoux1992, thomas2020,
salgarella2025, steele2017} rather than on independent Aegean-epigraphic
...
of our own. Chronological cross-validation is largely foreclosed because
roughly 74\% of the corpus sits in the single LM IB horizon, leaving
site-held-out splits as the only realistic partition. And the corpus itself
...
The framework is designed to withhold validation when a proposed reading
does not clear pre-registered negative controls, multiplicity adjustment,
contamination checks, and held-out evaluation; the contribution is the
...
\fi
The licensed epigraphic corpora --- GORILA-, SigLA-, lineara.xyz-, and
D\=AMOS-derived --- are not redistributed, per their CC BY-NC-SA /
all-rights-reserved terms; each source's licence and retrieval path is
documented (data-provenance), so the harness is reproducible against a
...
\emph{Replication detail for \S6--\S8. Nothing in these appendices is
load-bearing for the paper's argument, which stands on the body's headline
nulls; a reader who accepts those may skip them.}
...
segmentation induction over the full Linear A corpus (1,341 inscriptions
across 52 sites --- the 52 that survive logos normalization of Salgarella's
56-findplace envelope). Target claims are the recurring affixes and
positional grammar of the Davis/Thomas/Duhoux--Val\'{e}rio tradition:
prefixes and suffixes (\texttt{i-}, \texttt{a-}, \texttt{-te/-ti}, and
Davis's \texttt{i-*301} verb stem). Every candidate was graded against two
...
\texttt{a-}, \texttt{i-}, \texttt{-te/-ti}), a real confirm rate of 0.250
(Wilson 95\% CI [0.10, 0.50]) --- clearing the shuffle floor decisively.
The apparent standout \texttt{i-*301} sits far above the permutation null
(six distinct affix environments, $z \approx 8.0$ against the within-form
permutation null) --- but that figure is computed against the weaker
within-form permutation null only, and it does not survive the operative
\texttt{L\_fake} bigram floor. Under the bigram floor the whole set fails:
the fabricated Markov corpus confirms the same affixes at rate 0.375
(Wilson 95\% CI [0.18, 0.61]), above the real 0.250. The outcome is
classified NO POWER, not evidence against morphology;
\texttt{has\_morphology\_power} is False and the module reports the null
(the Linear-B positive control that validates the detector is reported
below).

\emph{Word-length distribution.} Linear A words are short --- mean 1.84
signs over all word-\emph{tokens}, median and mode 1, with 76.1\% of
word-tokens at $\le$2 signs (56.5\% single-sign) --- and on such words
``morphology'' and ``bigram order'' are statistically indistinguishable.
This reconciles with Fuls's 3.3-sign figure once the denominator is
matched: \citet[57, \S6.5]{fuls2015} computes the mean over a word
\emph{list} of ``554 complete sign sequences, where duplicates are reduced
to one'' (GORILA; \citealp{godart1985}) --- i.e. over distinct word
\emph{types}, not tokens. Restricting our corpus to distinct multi-sign
types likewise recovers 3.07; the token vs. type denominator is the whole
discrepancy. Fuls reads the 3.3-sign length, via his exponent-to-word-length
regression, as evidence that Minoan is \emph{polysynthetic}; our direct
morphology test finds the apparent affixation bigram-reproducible with no
power, so the word-length typology and the direct null point opposite ways,
and we report the null. The direct positive control --- the identical
bigram-floor test on Mycenaean Greek (Linear B), whose longer words carry
uncontested inflectional morphology --- was RUN on the D\=AMOS Linear-B
corpus (13,562 wordforms, mean 3.23 signs/word, parsed by the same
\texttt{\_damos\_wordforms} extractor as \S7.3) against a pre-registered
16-affix Mycenaean panel \citep{ventris1953}. It fires: REAL confirm rate
0.562 (9/16) --- carried by the securely-grammatical suffixes -jo, -o-jo
(gen sg -oio), -de (allative), -qe (enclitic ``and''), -o-i (dat pl) ---
against a within-word-shuffle floor $\sim$0.03 and an \texttt{L\_fake}
bigram floor $\sim$0.30 (stable over five draws), so
\texttt{has\_morphology\_power = True}. The identical test that returns NO
POWER on Linear A's short words recovers Mycenaean morphology on longer
ones (a sensitivity check: an ad-hoc parser yielding mean 2.19 signs/word
did \emph{not} fire --- power tracks word length exactly as the short-word
argument predicts), which rules out a universally insensitive detector and
supports the interpretation that Linear A's shorter forms contribute
materially to its lack of power
...
rate 0.406). A site-clustered bootstrap (resample the 52 sites with
replacement, 4,000 draws) puts the real-minus-random gap at a 95\% CI of
[0.021, 0.099], excluding zero (P(gap $>$ 0) = 0.998); the paired gap, not
the wide marginal per-segmenter CIs, is the operative test. Morfessor
over-segments the short syllabic corpus (micro-F1 0.156) and is reported as
not usable rather than hidden.

\emph{Stratified re-run.} The strata reconcile exactly to the corpus total:
290 admin + 99 libation + 129 other + 740 seal (structurally excluded as an
abbreviation channel) + 83 emptied by abbreviation-strip = 1,341
inscriptions. Induction ran on admin (290 inscriptions, 1,486 words, 14
sites), libation (99, 370, 15), and other (129, 187, 40). The
99-inscription ``libation'' \emph{genre} stratum --- stone-vessel
inscriptions over 15 sites --- is not the same object as the
libation-\emph{formula} corpus ($\sim$50 inscribed objects drawn from peak
sanctuaries \emph{and} caves, across more than five findspots). Following
...
False: no pre-registered affix is cross-stratum stable above the bigram
floor at the pre-registered $\alpha = 0.01$. Stratification also showed
...
\emph{Sensitivity, disclosed.} The honest 2$\times$2 is: under both
$\alpha$ thresholds at the defensible bucketing (libation = stone vessels
only), NULL; under loose bucketing (libation including stone objects and
inked), NULL at $\alpha = 0.01$ but at $\alpha = 0.05$ the affixes
\texttt{i-} (locative) and \texttt{-te/-ti} (ablative) validate. These are
...
genre choice. The pre-registered $\alpha = 0.01$ null is itself robust to
the bucketing, so the bucketing fix is not cherry-picking the null. logos's
own self-review caught the bigram free-rider \texttt{a-} and excluded it
via the per-affix floor; the adversarial reviewer also corrected a docstring
that mislabelled the $\ge$2-site presence gate as attempted-candidate-count
deflation. The present analytical approximation uses attempted-candidate
...
test) against a permutation null. On the real corpus (32 documents / 35
balance units), integer-level balance holds for 7/28 (25\%); of $\sim$7
fraction-bearing constraints only one is satisfied by the solved system,
and the sole determinable value is J = 1/2 (forced by HT104's
\texttt{2$\cdot$J == 1}). Held-out fraction balance is 0.0 against a
permutation-null mean of 0.113 (max 0.143), $p = 1.0$ --- not separated. A
synthetic positive control confirms this is not a machinery bug: with
planted values the solver recovers all of them and reaches held-out balance
0.9 vs null 0.22 ($p = 0.003$, separated). The null traces to sparse
...
= 1/2 is the long-standing consensus \citep{bennett1950}, with Schrijver's
\citeyearpar{schrijver2014} three further tablets (HT Zd 156, PE 1, HT
123a) lifting it to at least four corroborating documents.
...
Per-test results: consonant class (13 classes) at 0.080 accuracy (null
0.065, permutation $p = 0.40$) and vowel class (5 classes) at 0.140 (null
0.201, permutation $p = 0.86$). The honest verdict is \emph{data-limited},
not \emph{no bridge exists}, because the positive control disciplines the
reading. A first draft's flat positive control failed to fire at planted
strength 1.0; replacing it with a power curve over planted strengths shows
the test firing ($p < .05$) at strength $\ge$ 2 but not at 1. The test
...
signs to detect a weak one.'' Both point the same way: the sound path is
not recoverable from Linear A alone. The obvious next move --- impute sound
values cross-script from a larger Linear B corpus --- is now \emph{tested
rather than deferred}: \S7.3 reports that ingesting the full D\=AMOS
Mycenaean corpus leaves the cross-script distributional alignment at
chance, so more Linear B data is necessary but not sufficient; the block is
an absent distributional signal, not corpus volume alone. No sound value is
imputed for any sign. This pre-empts the predictable ``couldn't you just
use ML to recover the sounds?'' objection with a quantified detectability
floor rather than hand-waving.
...
+ shape-context) reach direct-NN accuracy 0.410 [0.18, 0.64] and Procrustes
0.185 [0.00, 0.36], far above image chance (0.0135); a from-scratch I-JEPA
reaches only $0.324 \pm 0.025$ (3 seeds) and is excluded from the claims as
over-parameterized for $\sim$90 glyphs. Image embeddings recover held-out
anchors where sequence co-occurrence sat at chance. Two controls decide how
to read that asymmetry, and together they \emph{downgrade} it from a
...
(direct-NN 0.367$\rightarrow$0.416, Procrustes 0.156$\rightarrow$0.195 on
the control's matched anchor set; both $\gg$ chance): the font-artifact
...
sign-shape similarity. An image encoder that ``recovers the A$\rightarrow$B
value map'' therefore re-derives a correspondence that was \emph{defined} by
shape --- the result is close to tautological and is not independent
evidence for any phonetic reading. We accordingly relabel this leg a
\textbf{shape-genealogy engineering demonstration, not a positive
decipherment claim}: it confirms a known palaeographic fact (borrowed signs
look alike) and validates a font-robust descriptor pipeline, but it reads no
Minoan. It remains truth-layer-capped in any case --- the
...
$\approx$15$\times$ the earlier cognate file, 56 anchors: the best of five
alignment methods reaches 0.025 against a 0.011 chance floor
(\texttt{clearly\_above\_chance = False}), while the \emph{positive control
recovers 0.947}, so the harness works and the null is a real null. Ingesting
all of D\=AMOS therefore reframes the obstacle from \emph{data volume} to
\emph{absent distributional signal}: the A$\leftrightarrow$B phonetic
correspondence is simply not carried by cross-script sign co-occurrence,
even at full Linear-B scale. And the A-only signs on which
positive-decipherment claims would pivot (e.g. *301) have no cross-script
anchor at all, so images cannot impute them even in principle.

\section{Contamination and sufficiency (detail)}

\paragraph{C.1 LLM-ablation reproduction gradient.} We ran the \S C.4
ablation against \texttt{qwen2.5:72b} --- a large open-weight model, a far
stronger memoriser than a local 12B --- on a rented H100 (40 seeds $\times$
40 held-out Linear A forms; NW-Semitic candidate; a Hebrew lexicon of 7,682
skeletons for the model-free arm; 0 LLM errors across 40/40 seeds)
(\artref). The first-pass contamination rate came out at \texttt{1.000} in
all 33 seeds where it was defined --- but this is an \textbf{artifact,
reported as such}: the mechanical arm never shares a literature-matching
correspondence with the LLM (their value vocabularies overlap in only 10 of
134 values), so the rate is forced to 1.0 by construction. Publishing it as
a contamination result would itself be the overfitting failure the
discipline layer exists to prevent (\artref).

The gate-2 rerun, reconciled to a common consonant-skeleton vocabulary,
yields the honest deliverable: a per-word reproduction table over 40 seeds.
\texttt{qwen2.5:72b} reproduces Gordon's \texttt{JA-NE} = \emph{yn} /
``wine'' in 14/40 seeds and Best 1981's \texttt{A-SA-SA-RA-ME} = ``oh
Asherah!'' in 2/40 --- published readings the model-free edit-distance
baseline never independently reaches (0/40) --- while never reproducing the
obscurer vessel equations \texttt{KA-RO-PA} and \texttt{SU-PA-RA} (0/40)
(\artref). Three of the four tested lexical items --- \texttt{JA-NE},
\texttt{KA-RO-PA}, \texttt{SU-PA-RA} --- are among Gordon's five
West-Semitic equations (as is the separately-treated \texttt{KU-RO} below);
the fourth, \texttt{A-SA-SA-RA-ME}, is Best's, not Gordon's. An external
red-team correction split \texttt{KU-RO}: of its 14/40 hits, 8/40 route
through the Semitic root \emph{kull} and 7/40 are reachable via the
...
downweighted (\artref). The surviving gradient --- famous readings
reproduced, obscure ones not --- is what C.4 detects: \textbf{a reproduction
gradient consistent with differential public exposure}, not isolating
memorisation from correlated item-level differences. The gate-2 rate itself
(mean 0.646) remains confounded by the baseline's representational ceiling
and is not reported as a contamination measure.

\paragraph{C.2 \texttt{L\_not\_indexed} abstention asymmetry.} The
\texttt{L\_not\_indexed} probe shows a sign in real context and asks whether
the model's behaviour on \emph{published} signs generalises to signs
\textbf{unseen in the indexed literature} --- absent from the indexed
...
assigned a value to the 20 known AB signs in $\sim$20/40 seeds but to the 11
not-indexed *-series signs in only $\sim$1/40 --- it abstains $\sim$97\% of
the time on the untouched signs while confidently valuing the memorisable
ones. The apparent known$-$not-indexed consistency gap (0.601, permutation
$p = 0.0005$) is coverage-confounded and was flagged, not asserted: the
matched-\emph{n} robustness check returned \texttt{no\_power} (0 of 11
not-indexed signs reached n $\ge$ 10), so the rigorous comparison could not
run. The interpretable signal is the abstention asymmetry itself, consistent
with differential public exposure (the experiment does not isolate
memorisation from correlated item-level differences) --- and, per the
module's standing rule, \textbf{consistency is not correctness}: with no
ground truth on the not-indexed signs, this bounds how the model behaves,
never whether it is right (\artref). The full arc cost $\approx$ \$2.80 in
ephemeral GPU rental.

\paragraph{C.3 Information-sufficiency CSA protocol.} The learning-curve
harness reframes the withdrawn unicity-distance diagnostic --- a toy-model
precondition, never a sufficiency or refutation claim --- as a sharper,
answerable question: \emph{is a Linear-A-scale corpus (539 distinct
word-forms; $V = 259$ sign-types at $N \approx 5{,}792$ parsed
syllable-signs) large enough to recover a decipherment IF a known cognate
existed?} We report the design and its reading rules here as a known-answer
upper-bound protocol; the full accuracy-vs-size curve, with per-size
permutation floors, is deposited in and completes in the repository, and
this paper asserts no interim result from it.

The protocol downsamples known-answer benchmarks --- Linear B / Mycenaean
Greek as the primary syllabic analog, sourced through D\=AMOS, and further
syllabic corpora --- and measures held-out cognate-ID recovery against a
permutation chance floor, using a published cognate-ID matcher
\citep{tamburini2025} that reports the \citet{luo2019} accuracy metric
\ifanon
(\artref).
\else
(\artref; \url{https://github.com/papadopouloskyriakos/logos}).
\fi
Because every benchmark is handed a real cognate signal Linear A lacks, any
recovery curve is an \textbf{optimistic benchmark, conditional on a known
cognate relationship} --- a best case Linear A does not enjoy --- readable
two ways: at-floor at Linear-A scale $\Rightarrow$ information-insufficient
even given a cognate; above-chance $\Rightarrow$ identifiability ---
cognate-absence and uncorrected multiplicity over a mixed sign inventory ---
rather than corpus size would be implicated. Neither branch is ever a
decipherability claim, and neither asserts ``there is enough text.'' Using a
data-size recovery curve to bound decipherability is not itself new ---
decipherment learning curves over cipher length \citep{ravi2008,
bergkirkpatrick2013}, the sample-size dependence of automatic cognate
detection \citep{list2014}, and analytic corpus-sufficiency thresholds
\citep{barber1974, zhang2016} are the ancestors; our contribution is the
specific repurposing to a cross-language cognate-ID sufficiency locator for
an undeciphered target, not the learning-curve idea.

The matcher is the parallel coupled-annealing path validated in the artifact
against its published known-answer benchmarks (a correctness check that the
reproduction reaches the published Table-3 values); the accuracy-vs-size
curve itself --- downsampling each benchmark and reading its recovery against
the per-size permutation floor under the two-way logic above --- is computed
and deposited in the repository, and this paper draws no Linear-A-scale
conclusion from it. The phonological / sound path, by contrast, is no longer
data-deferred: the D\=AMOS Mycenaean holdings are now harvested and
ingested, and \S7.3 reports that even the full corpus leaves the
cross-script distributional alignment at chance --- so that bound is a
property of the signal, not a promissory note against data yet to arrive. We
publish the harness and its bounding logic
\ifanon
as an anonymized artifact (released on acceptance).
\else
(concept DOI 10.5281/zenodo.21087572).
\fi

\paragraph{C.4 Information floor: corpus denominators and the unicity
computation (relocated from \S4).} The recurring sign-count confusion
resolves into four distinct denominators, each of which must be labelled
where it is used \citep[\S1--3]{salgarella2025}: \textbf{97} (Braovi\'{c}/Tan
token-frequent syllabograms), \textbf{$\sim$150} (full syllabary estimate),
\textbf{259} (the logos working repertoire, conflating syllabograms,
subscripts, and logograms), and \textbf{$\sim$350} (total including
ideograms; GORILA catalogues 390 signs). The channel through which
structure is observed is a \emph{sign-occurrence} count, not a document
count or a sign inventory (invariant 7), and two occurrence counts must be
kept distinct. The broader \textbf{$\sim$7,400 sign occurrences} across the
whole corpus \citep[Schoep 2002;][\S1--3]{salgarella2025} counts every
sign type --- syllabograms, logograms, and numerals alike. The narrower
\textbf{$N \approx 5{,}792$ parsed syllable-signs} is the subset that the
unicity computation below actually consumes, after logograms and numerals
are stripped. The two are different channel sizes and are used in
different places; the $\sim$7,400 figure never enters the unicity
arithmetic. Two further properties tighten the budget: Linear A words are
short and formulaic (the six-position libation-formula template;
\citealp[\S7]{salgarella2025}), and a single language/horizon dominates.

On the logos-normalized corpus (1,341 inscriptions, $N \approx 5{,}792$
parsed syllable-signs, $V = 259$), Shannon's unicity-distance criterion
yields a measured range of \textbf{$U \approx 204$--$415$ signs} across the
whole plausible $(V, P)$ parameter space, everywhere far below $N$.
\textbf{We retain this result only as a toy-model precondition diagnostic,
and explicitly withdraw it as any sufficiency or refutation claim.} It does
\emph{not} show there is enough text to pin a decipherment, and we do not
argue that. Unicity distance presupposes a \emph{known plaintext-language
oracle}; because we estimate the redundancy $D$ from the ciphertext itself,
that oracle collapses into Linear A up to relabelling, so the computation
demonstrates only measurable recurrent structure, not that any reading is
identifiable. The precondition it checks is necessary, not sufficient. And
it can fail to exist entirely: if Linear A underspells like Linear B, the
sign$\rightarrow$sound channel is lossy and non-injective, and such a
channel may possess no unicity distance at all. Corpus size, in short,
...
\[
  ...
  + \gamma\,\Phi^{-1}(1-\tfrac{1}{ne})\bigr]$}
\]
with $\gamma = 0.5772\ldots$ the Euler--Mascheroni constant --- the standard
blended approximation to the mean of the largest of $n$ draws
\citep[\emph{Order Statistics}, 3rd ed.]{david2003}; a candidate clears this
clause only if its observed held-out statistic exceeds the bar.
...
(overlapping sign-value assignments). Positive correlation makes the true
expected maximum \emph{smaller} than the independent-$n$ value, so using the
...
the approximation is therefore \textbf{conservative} under positive
correlation (it raises the bar and reduces false graduations). We use the
...
process. The interval on that calibration is the \textbf{one-sided exact
...
in 500 replicates, a 95\% upper bound of $\approx 1.54\%$.
\citet{bailey2014} enter only as selective-inference lineage (\S2.4), not as
the authority for this bar.
