...
def records(stream):
    ...
    for kind, payload in stream:
        ...
        out.append(dict(source_file="", doc_id=doc_id, site=site, support=support,
                        ...
                        is_kuro=1 if "KU-RO" in statement else 0, parser="C"))
    return out, dup_log
...
def main():
    ...
    for f in ("HTtexts.html", "misctexts.html", "religioustexts.html"):
        ...
        rows += rr
        dups += dd
    with open(os.path.join(HERE, "parser_c_output.csv"), "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=SCHEMA["fields"])
        w.writeheader()
        for r in rows:
            w.writerow(r)
    json.dump({"duplicates_logged": len(dups)},
              open(os.path.join(HERE, "parser_c_duplicates.json"), "w"))
    print(f"parser C: {len(rows)} records, "
