#!/usr/bin/env python3
"""gpu01_gpu_benchmark.py — one-cell CSA timing benchmark for the human to run on gpu01 (nllei01gpu01, RTX 3090 Ti).
...
PREREQUISITES on gpu01: the logos repo; the gitignored licensed benchmark corpora under corpus/bronze/
...
RUN (repo root), ~10-20 min total:
  ...
  python3 experiments/sufficiency/gpu01_gpu_benchmark.py --device cpu  --cell cypriot-greek:100:0
  # 2. GPU naive (device=cuda, unbatched) — expected SLOW (orchestration-bound):
  python3 experiments/sufficiency/gpu01_gpu_benchmark.py --device cuda --cell cypriot-greek:100:0
  # 3. GPU batched (one kernel/step, the built+validated path):
  python3 experiments/sufficiency/gpu01_gpu_benchmark.py --device cuda --batched --cell cypriot-greek:100:0
